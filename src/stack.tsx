		...
		<Stack.Navigator
			...
			screenOptions={{
				header: ({ scene, previous, navigation }) => {
					...
					const title =
						options.headerTitle !== undefined ? options.headerTitle : options.title !== undefined ? options.title : scene.route.name
					...
					return (
						<Appbar.Header theme={{ colors: { primary: theme.colors.surface } }}>
							...
							<Appbar.Content
								title={
									title === 'Feed' ? (
										<MaterialCommunityIcons style={{ marginRight: 10 }} name="twitter" size={40} color={theme.colors.primary} />
									) : (
										title
									)
								}
